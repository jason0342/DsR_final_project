...
movies <- read.csv("boxoffice2015.csv", header = F, stringsAsFactors = F); #read movie data
colnames(movies) <- c("title", "boxoffice", "date", "domestic");
for(i in c(1:50)) {
  infilename <- paste0("2015/n0", i, ".csv");
  df <- read.csv(infilename, header = F, skip = 1, na.strings = "");
  df <- df[,c(2,4,5)];
  colnames(df) <- c("tag", "day", "rate");
  df$tag <- sapply(df$tag, get_tag);
  df$day <- sapply(df$day, get_day, movies$date[i]);
  df$rate <- sapply(df$rate, get_nreply);
  write.csv(df, paste0("2015/processed/n", i, ".csv"), row.names = F, na = "");
}
...
weightedrating <- list();
for(i in c(1:50)) {
  infilename <- paste0("2015/processed/n", i, ".csv");
...
write.csv(movies, "2015/processed movie data.csv", row.names = F, na = "");
